...
class DiscreteCartPoleEnv(CartPoleEnv):
    def __init__(   self, 
                    sizes = (1,20,20,4),
                    steps = (1,2,45,192),
                    scales = (1,.1,.0001,.001),
    ...
    def step(self, *args, **keyArgs):
        observation, reward, terminated, truncated, info = super().reset(*args, **keyArgs)
        discrete_observation = self.discretize(observation)
        self.enumerated_state = self.enumerate_state(discrete_observation)
        self.state = np.array(self.undiscretize(discrete_observation))
        return np.array(self.state, dtype=np.float32), reward, terminated, truncated, info

class DiscreteCartPoleVectorEnv(CartPoleVectorEnv):
    def __init__(   self, 
                    sizes = (1,20,20,4),
                    steps = (1,2,45,192),
                    scales = (1,.1,.0001,.001),
...
class CustomDiscreteCartPoleEnv(CustomCartPoleEnv, DiscreteCartPoleEnv):
    def __init__(self, *args, **keyArgs):
        # super().__init__(*args, **keyArgs)
        CustomCartPoleEnv.__init__(self, *args, **keyArgs)
        DiscreteCartPoleEnv.__init__(self, *args, **keyArgs)

    def reset(self, *args, **keyArgs):
        CustomCartPoleEnv.reset(self, *args, **keyArgs)
        return DiscreteCartPoleEnv.reset(self, *args, **keyArgs)
    
    def step(self, *args, **keyArgs):
        return DiscreteCartPoleEnv.step(self, *args, **keyArgs)

class CustomDiscreteCartPoleVectorEnv(CustomCartPoleVectorEnv, DiscreteCartPoleVectorEnv):
    def __init__(self, *args, **keyArgs):
        # super().__init__(*args, **keyArgs)
        CustomCartPoleVectorEnv.__init__(self, *args, **keyArgs)
        DiscreteCartPoleVectorEnv.__init__(self, *args, **keyArgs)

    def reset(self, *args, **keyArgs):
        CustomCartPoleVectorEnv.reset(self, *args, **keyArgs)
        return DiscreteCartPoleVectorEnv.reset(self, *args, **keyArgs)
    
    def step(self, *args, **keyArgs):
        return DiscreteCartPoleVectorEnv.step(self, *args, **keyArgs)
